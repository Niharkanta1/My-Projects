    ...
    public class MainApp
    {
        ...
        public unsafe UnityEngine.Vector3 INTERNAL__getPosition(IntPtr addr, Memory mem)
        {
            try
            {
                ...
                {
                    logger.Info("Caught INTERNAL__getPosition exception - " + e.ToString());
                    pIndicesBufff.Free();
                    pMatricesBufff.Free();
                    return new UnityEngine.Vector3();
                }
            ...
            {
                logger.Info("GetBonePosition Crashed");
                return new UnityEngine.Vector3();
            }
        }

        public MainApp(UnityEngine.Vector2 win)
        {
            windowSize = win;
        }
        ...
        private MyRenderer rend = new MyRenderer();
        ...
        public void ReadWorlds()
        {
            while (true)
            {
                ...
                {
                    logger.Info("ReadWorlds crashed - " + e.ToString());
                    continue;
                }
            }
        }
