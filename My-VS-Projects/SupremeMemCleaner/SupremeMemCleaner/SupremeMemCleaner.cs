...
namespace SupremeMemCleaner
{
    ...
    public class SupremeMemCleaner
    {
        ...
        public void DrawWithAimAndDistance(WindowRenderTarget device)
        {
            rend.device = device;
            SolidColorBrush solidColorBrush = new SolidColorBrush(device, SharpDX.Color.Gray);
            TextFormat espFont = new TextFormat(new FontFactory(), "Tahoma Bold", 10f);
            ...
            foreach (Player p in players)
            {
                ...
                float new_Distance = UnityEngine.Vector3.Distance(players[0].GetMovementContext().GetLocation(), p.GetMovementContext().GetLocation());
                ...
                if (new_Distance != 0)
                    headSZ = 2 + (20 / new_Distance);
                double distance = Dist2D(centerX, centerY, headW2S.x, headW2S.y);
                if (distance < lowDist && new_Distance < 150)
                {
                    lowDist = distance;
                    aimPosX = headW2S.x;
        ...
        public void DrawWithAimForScav(WindowRenderTarget device)
        {
            rend.device = device;
            SolidColorBrush solidColorBrush = new SolidColorBrush(device, SharpDX.Color.Gray);
            TextFormat espFont = new TextFormat(new FontFactory(), "Tahoma Bold", 10f);
